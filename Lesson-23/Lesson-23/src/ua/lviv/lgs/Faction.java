...
//		System.out.println("We can`t find ");
		...
		delegates.stream().forEach(delegate -> {
			if (delegate.getName().equals(delegateName) && delegate.getSurname().equals(delegateSurName)) {
				delegate.takeBrube(brube);
				return;
			} else {
				System.out.println("We can`t find ");
			}
	...
	public void addDelegate(String delegateName, String delegateSurname) {
		...
		delegates.forEach(delegate -> System.out.println(delegate));
	...
	public void selectBriberInFaction() {
		...
		delegates.stream().forEach(delegate -> {
			...
			else {System.out.println("Хабарників не знайдено");}
		});
	...
	public void selectAllDelegate() {
		...
		delegates.stream().forEach(delegate -> System.out.println(delegate));
